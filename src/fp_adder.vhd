-- =====================================================================
-- fp_adder.vhd
-- Somador de ponto flutuante simplificado (13 bits)
-- Formato: 1 bit sinal + 4 bits expoente + 8 bits significando (fracao)
-- Valor = (-1)^s * 0.f * 2^e
--
-- Fonte: Pong P. Chu, "FPGA Prototyping by VHDL Examples", Listing 3.19.
-- ETAPA 1: transcricao fiel do PDF (sem alteracoes na logica).
--
-- Sufixos usados nos nomes dos sinais:
--   b = big (numero maior)      s = small (numero menor)
--   a = aligned (alinhado)      n = normalized (normalizado)
-- =====================================================================
library ieee;
...
begin
    ...
    -- 1o estagio: ordenar para achar o numero maior
    ...
    -- 2o estagio: alinhar o numero menor (deslocar fracs a direita)
    exp_diff <= expb - exps;
    ...
    -- 3o estagio: somar / subtrair
    sum <=
        ('0' & fracb) + ('0' & fraca) when signb = signs else
        ('0' & fracb) - ('0' & fraca);

    -- 4o estagio: normalizar
    -- conta os zeros a esquerda (leading zeros)
    ...
    -- desloca o significando conforme os zeros a esquerda
    with leado select
        sum_norm <=
    ...
    -- normalizacao com condicoes especiais
    process (sum, sum_norm, expb, leado)
    begin
        if sum(8) = '1' then
            -- houve carry out: desloca frac a direita e incrementa exp
            expn  <= expb + 1;
            fracn <= sum(8 downto 1);
        elsif (leado > expb) then
            -- pequeno demais para normalizar: vira zero
            expn  <= (others => '0');
            fracn <= (others => '0');
        else
            expn  <= expb - leado;
